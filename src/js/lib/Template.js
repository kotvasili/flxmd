...
const videoCover = videocover => {
  return `<div class="portfolio__item-videocover">
			<img class='js-image' data-src="${videocover}" >
		</div>`;
};
...
export const Template = (template) => {
  if(template.image) {
    return `<div id=${template.id} class="grid__item w__${template.width} load-parent crsor-trgr" data-cursor-type="plus">
			<div class="portfolio__item touch-down not-move" style='background-color: ${template.bgimage}'>
				<a href="${template.internal}" target="_blank" class="portfolio__item-img">
  ...
  if(template.video) {
    return `<div id=${template.id} class="grid__item w__${template.width} load-parent crsor-trgr" data-cursor-type="plus">
			<div class="portfolio__item touch-down not-move" style="background-color: ${template.bgimage}">
				<a href="${template.internal}" target="_blank" class="portfolio__item-video">
				...
				${ template.videocover !== undefined ? videoCover(template.videocover): false}
			</div>
		</div>`;
  }
};
